...
impl ResolvingGraph {
    ...
    fn build_resolution_graph(
            ...
            .into_iter()
        {
            match resolution_table.get(&name) {
                Some(other) => {
                    other.resolve(addr_opt).with_context(|| {
                        format!(
        ...
        if self.build_options.dev_mode && is_root_package {
                ...
                .into_iter()
            {
                match resolution_table.get(&name) {
                    Some(other) => {
                        other.resolve(Some(addr)).with_context(|| {
                            format!(
                                "Unable to resolve named address '{}' in\
    ...
    fn process_dependency(
        ...
        dep: Dependency,
        root_path: PathBuf,
    ) -> Result<(Renaming, ResolvingTable, GraphIndex)> {
            ...
            for (name, rename_from_or_assign) in dep_subst.into_iter() {
                match rename_from_or_assign {
                    ...
                    SubstOrRename::Assign(value) => {
                        resolution_table
                            .get(&name)
                            .map(|named_addr| named_addr.resolve(Some(value)))
                            .transpose()?;
                    }
                }
            }
...
impl ResolvingNamedAddress {
    ...
    pub fn resolve(&self, address_opt: Option<AccountAddress>) -> Result<()> {
        match address_opt {
            None => Ok(()),
